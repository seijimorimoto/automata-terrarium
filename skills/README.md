...
| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/log`](log/) | ✅ | ⚠️ | Append a timestamped work entry to the current week's log |
| [`/weekly-status`](weekly-status/) | ✅ | ⚠️ | Generate a weekly status report from ADO, Todoist, and local log |

### GitHub Quick PR

| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/quick-pr`](quick-pr/) | ✅ | ⚠️ | Create a branch, commit, push, open a GitHub PR, squash-merge, and clean up |
...
| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/coverage-check`](coverage-check/) | ✅ | ⚠️ | Detect the project's coverage tool, run coverage scoped to the diff, classify each uncovered chunk by kind (`pure_logic` / `trivial` / `untestable` / `generated`); emits JSON findings |
...
| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/doc-review`](doc-review/) | ✅ | ⚠️ | Inspect the diff for missing/stale documentation and emit report-only JSON findings (doc accuracy, stale references, missing doc-comments, suggest-new-docs, index linkage) |
...
| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/standards-check`](standards-check/) | ✅ | ⚠️ | Discover a repo's standards files, extract rules, check the diff against them; emits tiered JSON findings (`hard_block` / `soft_block` / `report`) |
...
| Skill | Claude Code | Copilot CLI | Description |
|-------|-------------|-------------|-------------|
| [`/implement`](implement/) | ✅ | ⚠️ | Carry out an approved plan: branch (or worktree), per-step commits, draft PR via the right PR-creation skill |
...
Copy-Item -Recurse skills\* ~\.copilot\skills\
```
