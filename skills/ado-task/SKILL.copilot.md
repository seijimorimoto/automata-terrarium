...
Read `~/.copilot/work-status-config.json` when present. During migration, if that file is missing, also check `~/.claude/work-status-config.json` and tell the user which path was used.

If missing, tell the user:

> Configuration file not found. Please create `~/.copilot/work-status-config.json`:
> ```json
> {
...
Parse the first word of the user's request as the subcommand. The rest is arguments to that subcommand.
...
- Copilot MCP tools may be exposed with names such as `ado/<tool-name>` rather than Claude-style `mcp__ado__<tool-name>`. Use the available equivalent tool.
